...
use crate::dns::netutils::{read_packet_length, write_packet_length};
use crate::dns::protocol::{DnsPacket, QueryType};
use chrono::*;
use derive_more::{Display, Error, From};

use super::buffer::{BytePacketBuffer, PacketBuffer, StreamPacketBuffer};
use super::protocol::DnsQuestion;
...
/// The UDP client.
///
/// This includes a fair bit of synchronization due to the stateless feature of UDP.
/// When many queries are sent in parallell, the response packets can com back
...
impl DnsNetworkClient {
    ...
    /// method is thread safe, and can be used from any number of threads in parallell.
...
impl DnsClient for DnsNetworkClient {
    ...
    /// response will ever be generated, and clients will just block indefinitely.
    fn run(&self) -> Result<()> {
                        ...
                        if let Ok(mut pending_queries) = pending_queries_lock.lock() {
                            ...
                            for (i, pending_query) in pending_queries.iter().enumerate() {
                                if pending_query.seq == packet.header.id {
                                    // Matching query found, sned the response.
                                    let _ = pending_query.tx.send(Some(packet.clone()));

                                    // Mark this index for remove from list
    ...
    fn send_query(
        ...
        println!("Truncated response - sending as TCP");
