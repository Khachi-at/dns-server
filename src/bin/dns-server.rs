use dns_server::dns::context::{ResolveStrategy, ServerContext};
use dns_server::dns::protocol::{DnsRecord, TransientTtl};
use dns_server::dns::server::{DnsServer, DnsTcpServer, DnsUdpServer};
use dns_server::web::server::WebServer;
use getopts::Options;
...
fn main() {
    ...
    opts.optflag(
        "a",
        "authority",
        "disable support for recursive lookups, and serv only local zones",
    );
    opts.optopt(
        "f",
        "forward",
        "forwward replies to specified dns server",
        "SERVER",
    );
